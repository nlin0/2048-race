...
def train(
    *,
    ...
    env = Game2048Env(
        seed=env_seed,
        max_steps=50_000,
        anchor_corner=anchor_corner,  # type: ignore[arg-type]
        legacy_corner_shaping=legacy_corner_shaping,
        edge_fill_bonus=edge_fill_bonus,
        max_log2_increase_bonus=max_log2_increase_bonus,
        danger_axis_penalty=danger_axis_penalty,
    )
    print(
        f"Game2048Env: anchor_corner={anchor_corner} "
        f"legacy_corner_shaping={legacy_corner_shaping} "
        f"edge_fill_bonus={edge_fill_bonus} max_log2_increase_bonus={max_log2_increase_bonus} "
        f"danger_axis_penalty={danger_axis_penalty}"
    )
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default=0.01,
        help=(
            "Floor exploration rate after linear decay. Low values (e.g. 0.01, or 0.001) "
            "reduce late-game RNG so the policy relies more on the value network."
        ),
    )
    ...
    p.add_argument(
        ...
        choices=["bl", "br", "tl", "tr"],
        default="bl",
        help="Fixed anchor for corner / monotonicity / edge shaping (matches training)",
    )
    p.add_argument(
        "--legacy-corner-shaping",
        action="store_true",
        help=(
            "Match older reward: any-corner max + best-of-four monotonicity; "
            "disables anchor edge fill, max-tile-increase bonus, and danger-axis penalty"
        ),
    )
    p.add_argument(
        "--edge-fill-bonus",
        type=float,
        default=0.05,
        help="Weight for mean fill on the two edges meeting the anchor (0 to disable)",
    )
    p.add_argument(
        "--max-log2-increase-bonus",
        type=float,
        default=0.12,
        help="Scale for reward when global max tile increases (log2 difference)",
    )
    p.add_argument(
        "--danger-axis-penalty",
        type=float,
        default=0.04,
        help="Subtracted when taking the 'away from anchor' slide while max stayed on anchor",
    )
    args = p.parse_args()
    ...
    train(
        ...
        legacy_corner_shaping=args.legacy_corner_shaping,
        edge_fill_bonus=args.edge_fill_bonus,
        max_log2_increase_bonus=args.max_log2_increase_bonus,
        danger_axis_penalty=args.danger_axis_penalty,
    )
